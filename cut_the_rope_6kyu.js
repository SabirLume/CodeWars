...
function cutTheRopes(ropes) {
    let ropesSorted = ropes.sort(( a, b ) => a - b )
    let final = [ropes.length];
    let r = [];
  
    //stop when all values are 0 until then repeat!
    while(ropesSorted.length > 0) {
      ropesSorted.forEach(rope => {    
      //subtrack the smallest
       r.push(rope - ropesSorted[0])
     })
      //take out any zeros
      r = r.filter(x => x > 0) 
      ropesSorted = r
  
      //record length of the array 
      if(r.length > 0){
        final.push(r.length)
      }
        r = []
